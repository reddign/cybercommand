...
function getTable($name) {
    switch($name) {
    case 'coaching':
        $coachingTable = new Table('coaching', ['Coaching ID','Student ID','Date','Visit Type','For Coursework','Mode','Reason','Position Type','Follow Up Tasks', 'Deadline', 'Notes'], ['studentID', 'date']);
    ...
    case 'contact':
        $contactTable = new Table('contact', ['Contact ID','Company ID','First Name','Last Name','Job Title','Contact Type','Email','Phone Number','Primary Contact','Engagement Level','Etown Priority Partner','Company Domain','Industry','Major/Concentrations','Notes'], ['firstName','lastName']);
    ...
    case 'first_destination':
        $first_destinationTable = new Table('first_destination', ['First Destination ID','Company ID','Student ID','Title','Location','Salary Range','Offer Date','What are you doing after graduation','In EMCS Network','Did you intern at this company?','Relationship to Major(s) and Minor(s)','Matches Career Path','Department','Notes'], ['title']);
        $first_destinationTable->fileName = "first_destinations.php";
        return $first_destinationTable;

    case 'internship':
        $internshipTable = new Table('internship', ['Internship ID','Student ID','Company ID','Title','Department','Experimental Learning','Work-based Learning','Term','SLE/Credit','Career Path','Mode','Rating','Wage Range','EMCS Network','Notes'], ['title']);
    ...
    case 'meeting':
        $meetingTable = new Table('meeting', ['Meeting Type','Meeting Type','Date','Company ID','Contact ID','Etown Contact','Notes','Tasks'], ['meetingType','date']);
